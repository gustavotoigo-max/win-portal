...
function serverSecret() {
  return process.env.LICENSE_HMAC_SECRET || "dev-only-secret";
}
...
export function buildLicensePayload({
  ...
  const validationUtc = now.toISOString();
  const expiresAtUtc = license.expires_at ? new Date(license.expires_at).toISOString() : null;

  return {
    license_id: license.id,
    activation_id: activation.id,
    app_id: license.app_id || process.env.LICENSE_APP_ID || DEFAULT_APP_ID,
    product_id: license.product_id || product?.id || null,
    software: product?.aliases?.[0] || product?.name || null,
    email,
    ...
    last_validation_utc: validationUtc,
    last_server_validation_utc: validationUtc,
    status: "ACTIVE",
